...
static void reconnect_wait_verify_disconnect(void)
{
	int err;
...
static void reconnect(void)
{
	...
	/* First connection: scan first so we are ready when the peripheral advertises. */
	err = bt_le_scan_start(BT_LE_SCAN_PASSIVE, NULL);
	TEST_ASSERT(err == 0, "phase2 bt_le_scan_start failed (%d)", err);

	bk_sync_send();

	reconnect_wait_verify_disconnect();

	/* Second connection: peripheral soft-resets the stack, then advertises again */
	err = bt_le_scan_start(BT_LE_SCAN_PASSIVE, NULL);
	TEST_ASSERT(err == 0, "bt_le_scan_start (after soft reset) failed (%d)", err);
