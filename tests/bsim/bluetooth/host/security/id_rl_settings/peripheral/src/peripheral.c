...
static void soft_reset_stack(void)
{
	int err;

	err = bt_disable();
	TEST_ASSERT(err == 0, "bt_disable (soft reset) failed (%d)", err);

	err = bt_enable(NULL);
	TEST_ASSERT(err == 0, "bt_enable (soft reset) failed (%d)", err);

	/* Auth info callbacks stay linked across bt_disable()/bt_enable(); do not register again. */
	err = settings_load();
	TEST_ASSERT(err == 0, "settings_load after soft reset failed (%d)", err);
}
...
static void directed_connect_cycle(void)
{
	int err;
...
static void directed_reconnect(void)
{
	...
	/* First connection after cold boot from flash */
	directed_connect_cycle();

	soft_reset_stack();

	/* Second connection: RL/keys intact after stack soft reset */
	directed_connect_cycle();
}
