...
static uint8_t _listen(uint16_t psm, uint16_t response, uint8_t mode,
		       uint32_t options)
{
...
static uint8_t listen(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
	...
	return _listen(psm, response, BTP_L2CAP_LISTEN_V2_MODE_NONE, 0);
}
...
static uint8_t listen_v2(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
	...
	return _listen(psm, response, cp->mode, options);
}
